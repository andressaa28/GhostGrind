...
class PacMan:
    ...
    def scoreboard(self):
        ...
        x_pos = self.window.get_width() - max(texto_instrucao_r.get_width(), texto_instrucao_esc.get_width()) - margem
    ...
    while True:  # Loop da partida
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                ...
                if tecla == 'escape':  # Fecha o jogo
                    pg.quit()
                    quit()

                elif tecla == 'r':  # Volta para tela inicial
                    sair_para_menu = True
